...
public class InstanceUploaderTask extends AsyncTask<Long, Integer, HashMap<String, String>> {
	...
	public void setLogon( String mLogon) {
		this.mLogon = mLogon;
	}
	...
	protected HashMap<String, String> doInBackground(Long... values) {
		...
		final DefaultHttpClient defaultHttpclient = new DefaultHttpClient(params);
		//CH Added Basic auth
			...
			HttpRequestInterceptor preemptiveAuth = new HttpRequestInterceptor() {
				public void process(final HttpRequest request, final HttpContext context) throws HttpException, IOException {
					...
					if (authState.getAuthScheme() == null) {
						...
						UsernamePasswordCredentials usernamePassword = new UsernamePasswordCredentials(mLogon, mPassword);
						defaultHttpclient.getCredentialsProvider().setCredentials(authScope, usernamePassword);
						//						if (creds != null) {
		...
		try {
			...
			if (c.getCount() > 0) {
				...
				while (c.moveToNext()) {
					...
					if ( !uploadOneSubmission(urlString, id, instance, toUpdate, defaultHttpclient, localContext, uriRemap) ) {
